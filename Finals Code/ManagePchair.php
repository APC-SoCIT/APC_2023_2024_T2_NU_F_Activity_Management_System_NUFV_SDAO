...
<head>
  ...
  <title>Program Chair's Account</title>
</head>
...
<div class="split-background">
        <div class="left">
            ...
            <ul>
             ...
             <li><a href="ManagePchair.php" class="form"><img src="" class="statusIcon">Program Chair Accounts</a></li>
    ...
    <div class="right">
        ...
        <h1 class="heading">Program Chair Accounts</h1><br>
        <button class="btn-new" onclick="location.href='NewPchair.php'">Create New</button><br><br>
  <div class="container">
    <?php
    if (isset($_GET["msg"])) {
      $msg = $_GET["msg"];
    ...
    <table>
      ...
      <tbody>
        <?php
        $sql = "SELECT * FROM `program_chair_approver`";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
        ?>
          <tr>
            <td><?php echo $row["id"] ?></td>
            <td><?php echo $row["program_chair_name"] ?></td>
            <td><?php echo $row["program_chair_email"] ?></td>
            <td><?php echo $row["program_chair_program"]?></td>
            <td>
            <div class="button-container">
         <button class="btn btn-edit" onclick="window.location.href='EditPchair.php?id=<?php echo $row["id"] ?>'">
            <i class="fa-solid fa-solid fa-trash fs-5"></i>Update
        </button> <br><br>
        <?php
        echo "<a href='DeletePchair.php?id={$row["id"]}' onclick=\"return confirm('Are you sure to delete this account?');\" class='btn btn-delete' style='text-decoration: none;'>";
