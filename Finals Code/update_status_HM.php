...
if (isset($_POST['org'])) {
    ...
    $sqlHmStatus = "UPDATE sarf.status_of_requests SET hm = 'approved' WHERE user_id = $userId";
    $resultHm = $conn->query($sqlHmStatus);

    if (!$resultHm) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET hm_dt_response = NOW() WHERE user_id = $userId";
