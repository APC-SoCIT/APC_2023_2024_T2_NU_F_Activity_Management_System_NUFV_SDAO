...
<div class="split-background">
    ...
    <div class="right">
        ...
        <h1 class="heading">Organization Adviser Accounts</h1><br>
        <button class="btn-new" onclick="location.href='NewOAdviser.php'">Create New</button><br><br>
  <div class="container">
    <?php
    if (isset($_GET["msg"])) {
      $msg = $_GET["msg"];
    ...
    <table>
      ...
      <tbody>
        <?php
        $sql = "SELECT * FROM `org_adviser_approver`";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
        ?>
          <tr>
            <td><?php echo $row["id"] ?></td>
            <td><?php echo $row["org_adviser_name"] ?></td>
            <td><?php echo $row["org_adviser_email"] ?></td>
            <td><?php echo $row["org_adviser_program"]?></td>
            <td>
            <div class="button-container">
         <button class="btn btn-edit" onclick="window.location.href='EditOAdviser.php?id=<?php echo $row["id"] ?>'">
            <i class="fa-solid fa-solid fa-trash fs-5"></i>Update
        </button> <br><br>
        <?php
        echo "<a href='DeleteOAdviser.php?id={$row["id"]}' onclick=\"return confirm('Are you sure to delete this account?');\" class='btn btn-delete' style='text-decoration: none;'>";
