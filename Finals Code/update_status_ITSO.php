...
if (isset($_POST['org'])) {
    ...
    $sqlItsoStatus = "UPDATE sarf.status_of_requests SET itso = 'approved' WHERE user_id = $userId";
    $resultItso = $conn->query($sqlItsoStatus);

    if (!$resultItso) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET itso_dt_response = NOW() WHERE user_id = $userId";
