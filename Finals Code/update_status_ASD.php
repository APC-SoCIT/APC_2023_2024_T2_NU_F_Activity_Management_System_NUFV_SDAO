...
if (isset($_POST['org'])) {
    ...
    $sqlAsdStatus = "UPDATE sarf.status_of_requests SET asd = 'approved' WHERE user_id = $userId";
    $resultAsd = $conn->query($sqlAsdStatus);

    if (!$resultAsd) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET asd_dt_response = NOW() WHERE user_id = $userId";
