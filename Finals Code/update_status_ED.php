...
if (isset($_POST['org'])) {
    ...
    $sqlEdStatus = "UPDATE sarf.status_of_requests SET ed = 'approved' WHERE user_id = $userId";
    $resultEd = $conn->query($sqlEdStatus);

    if (!$resultEd) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET ed_dt_response = NOW() WHERE user_id = $userId";
