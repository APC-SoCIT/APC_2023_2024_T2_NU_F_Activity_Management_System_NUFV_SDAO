...
if (isset($_POST['org'])) {
    ...
    $sqlSdaoStatus = "UPDATE sarf.status_of_requests SET sdao = 'approved' WHERE user_id = $userId";
    $resultSdao = $conn->query($sqlSdaoStatus);

    if (!$resultSdao) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET sdao_dt_response = NOW() WHERE user_id = $userId";
