...
if (isset($_POST['org'])) {
    ...
    $sqlLrcStatus = "UPDATE sarf.status_of_requests SET lrc = 'approved' WHERE user_id = $userId";
    $resultLrc = $conn->query($sqlLrcStatus);

    if (!$resultLrc) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET lrc_dt_response = NOW() WHERE user_id = $userId";
