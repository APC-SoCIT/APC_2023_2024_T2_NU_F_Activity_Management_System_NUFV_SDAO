...
if (isset($_SESSION['email']) && isset($_SESSION['password'])) {
    ...
    if($valueOfEmail == 'asdirector@nu-fairview.edu.ph'){
        ...
        $sql = "SELECT f.filename ,f.upload_date, f.activity_title, s.activity_types, s.program_organization 
        FROM files f 
        JOIN sarf_requests s ON f.activity_title = s.activity_title WHERE amount > 2000.00";

}
    else if($valueOfEmail == 'edirector@nu-fairview.edu.ph'){
        $sql = "SELECT f.filename ,f.upload_date, f.activity_title, s.activity_types, s.program_organization 
        FROM files f 
        JOIN sarf_requests s ON f.activity_title = s.activity_title WHERE activity_types != 'Internal (within the campus)' 
        OR amount > 5000.00";
