...
<head>
  ...
  <title>Director's Account</title>
</head>
...
<div class="split-background">
    ...
    <div class="right">
        ...
        <h1 class="heading">Director Accounts</h1><br>
        <button class="btn-new" onclick="location.href='NewDirector.php'">Create New</button><br><br>
  <div class="container">
    <?php
    if (isset($_GET["msg"])) {
      $msg = $_GET["msg"];
    ...
    <table>
      ...
      <tbody>
        <?php
        $sql = "SELECT * FROM `directors_approver`";
        $result = mysqli_query($conn, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
        ?>
          <tr>
            <td><?php echo $row["id"] ?></td>
            <td><?php echo $row["dapprover_name"] ?></td>
            <td><?php echo $row["dapprover_email"] ?></td>
            <td><?php echo $row["dapprover_position"]?></td>
            <td>
            <div class="button-container">
         <button class="btn btn-edit" onclick="window.location.href='EditDirector.php?id=<?php echo $row["id"] ?>'">
            <i class="fa-solid fa-solid fa-trash fs-5"></i>Update
        </button> 
        <?php
        echo "<a href='DeleteDirector.php?id={$row["id"]}' onclick=\"return confirm('Are you sure to delete this account?');\" class='btn btn-delete' style='text-decoration: none;'>";
