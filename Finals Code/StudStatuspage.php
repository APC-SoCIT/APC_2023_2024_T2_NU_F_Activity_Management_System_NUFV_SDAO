...
if (isset($_SESSION['email']) && isset($_SESSION['password'])) {
    $valueOfEmail = $_SESSION['email'];
        ...
        $conn2 = new mysqli($usersDBHost, $usersDBUser, $usersDBPassword, $usersDBName);
        ...
        $sqlUserId = "SELECT id, program_organization FROM nu_accountsdb.studentaccounts WHERE email = '$valueOfEmail'";
        $resultUserId = $conn2->query($sqlUserId);
        if (!$resultUserId) {
            die("Query failed: " . $conn2->error);
        }
        $rowUserId = $resultUserId->fetch_assoc();
        $userId = $rowUserId['id'];
        $org = $rowUserId['program_organization'];
        ...
        $checkID = "SELECT user_id FROM sarf.status_of_requests WHERE user_id = $userId";
        $resultCheckID = $conn->query($checkID);
        if($resultCheckID->num_rows > 0){
            $styleForDivContent = '';
            $styleForDivContent2 = 'display: none;';
       ...
       if ($OrgAdvStatus == 'approved') {
        ...
        $styleForDivContent = 'display: none;';
        $styleForDivContent2 = '';
...
.profilebtn:hover::after {
    ...
    font-size: 14px;
        ...
        <div class="title">
            ...
            <div class="contentForNoStatus" style="<?php echo $styleForDivContent2 ?>">
            <h2>Your organization currently doesn't have any pending activities.</h2>
            </div>
            <div class="Content" style="<?php echo $styleForDivContent ?>">
