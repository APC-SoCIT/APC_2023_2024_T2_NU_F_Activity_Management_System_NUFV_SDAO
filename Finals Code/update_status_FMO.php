...
if (isset($_POST['org'])) {
    ...
    $sqlFmoStatus = "UPDATE sarf.status_of_requests SET fmo = 'approved' WHERE user_id = $userId";
    $resultFmo = $conn->query($sqlFmoStatus);

    if (!$resultFmo) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET fmo_dt_response = NOW() WHERE user_id = $userId";
