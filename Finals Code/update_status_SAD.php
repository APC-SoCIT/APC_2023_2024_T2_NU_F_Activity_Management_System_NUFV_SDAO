...
if (isset($_POST['org'])) {
    ...
    $sqlSadStatus = "UPDATE sarf.status_of_requests SET sad = 'approved' WHERE user_id = $userId";
    $resultSad = $conn->query($sqlSadStatus);

    if (!$resultSad) {
        die("Query failed: " . $conn->error);
    }

    $sqlinsertDtResponse = "UPDATE sarf.status_response SET sad_dt_response = NOW() WHERE user_id = $userId";
